...
export default function UnitsTab() {
  ...
  const handleDataChange = (newData: UnitDataInfo[]) => {
    ...
    for (const r of finalData) {
      const activeVal =
        r.active === "act" || r.active === "Active"
          ? "Active"
          : r.active === "Unactive" || r.active === "unact"
            ? "Unactive"
            : r.active || "Active";
      ...
      if (activeRows.length > 1) {
        ...
        if (newlyUpdated) {
          ...
          if (confirmed) {
            ...
            if (idx > -1 && units[idx]) {
              finalData[idx] = { ...units[idx] }; // Revert back to old row (clone to force re-render)
            }
          }
        }
      }
  ...
  const getUniqueCount =
    (key: keyof UnitDataInfo) => (filteredData: UnitDataInfo[]) => {
      ...
      (
        ...
        return (
          <AutocompleteCell
            value={val || ""}
            onChange={(newVal) => {
              // Do not update data on every keystroke to prevent premature duplicate detection
            }}
  ...
  const renderNumericCell = React.useCallback(
    (key: keyof UnitDataInfo) =>
      (
        val: any,
        row: UnitDataInfo,
      ...
      (
        ...
        <select
          ...
          className={cn(
            ...
            val === "Unactive" || val === "unact"
              ? "text-red-400"
              : "text-green-400",
            isLocked
              ? "bg-transparent opacity-50 cursor-not-allowed"
              : "cursor-pointer bg-slate-900/80 hover:bg-slate-800 transition-colors focus:bg-brand-600/20 rounded px-1 min-w-[70px] py-1.5 shadow-inner shadow-black/40 border border-slate-700/50 hover:border-slate-500 focus:border-brand-500/50",
          )}
        >
          <option
            value="Active"
    ...
    () => [
      {
        ...
        renderCell: renderTextCell("floor"),
      },
      {
        key: "unit",
        label: "Unit",
    ...
    () => ({
      ...
      mapping: (row: any) => {
        ...
        return {
          ...
          active:
            activeVal.toLowerCase().includes("unact") ||
            activeVal.toLowerCase().includes("inact")
              ? "Unactive"
              : "Active",
        };
      },
