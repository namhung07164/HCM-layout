  ...
  const renderTextCell = React.useCallback((key: keyof UnitDataInfo) => (val: any, row: UnitDataInfo, updateRow: (newRow: UnitDataInfo) => void, isLocked: boolean) => {
    ...
    return (
      <AutocompleteCell 
        ...
        onSelect={(item) => updateRow({ ...row, [key]: typeof item === 'object' ? item.value : item })}
        options={options}
        minChars={0}
        isLocked={isLocked}
        placeholder="..."
  ...
  const renderActiveCell = React.useCallback(() => (val: any, row: UnitDataInfo, updateRow: (newRow: UnitDataInfo) => void, isLocked: boolean) => (
    <select
      ...
      className={cn(
        ...
        isLocked ? "bg-transparent opacity-50 cursor-not-allowed" : "cursor-pointer bg-slate-900/80 hover:bg-slate-800 transition-colors focus:bg-brand-600/20 rounded px-1 min-w-[70px] py-1.5 shadow-inner shadow-black/40 border border-slate-700/50 hover:border-slate-500 focus:border-brand-500/50"
      )}
    >
